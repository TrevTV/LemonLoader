...
void Mono::CreateDomain()
{
	if (Domain == NULL)
	{
		...
		mono_runtime_set_main_args(LemonLoader::CommandLineC, LemonLoader::CommandLineV);
		Domain = mono_jit_init("LemonLoader");
		Mono::FixDomainBaseDir();
		mono_debug_init(MONO_DEBUG_FORMAT_MONO);
		mono_debug_domain_create(Domain);
		mono_jit_parse_options(LemonLoader::CommandLineC, LemonLoader::CommandLineV);
	}
}
...
void Mono::FixDomainBaseDir()
{
	...
	if (!IsOldMono)
		mono_domain_set_config(Domain, LemonLoader::GamePath, "LemonLoader");
}
