...
void Console::Create()
{
	if (!IsInitialized())
	{
		if (AllocConsole())
		{
			...
			SetTitle(("LemonLoader " + (LemonLoader::DebugMode ? std::string("Debug") : std::string("Normal")) + " Console").c_str());
			SetForegroundWindow(hwndConsole);
			freopen_s(reinterpret_cast<FILE**>(stdout), "CONOUT$", "w", stdout);
		}
		else
			MessageBox(NULL, ("Failed to Create the " + (LemonLoader::DebugMode ? std::string("Debug") : std::string("Normal")) + " Console!").c_str(), NULL, MB_OK | MB_ICONEXCLAMATION);
	}
}
