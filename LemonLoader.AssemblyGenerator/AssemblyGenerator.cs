...
using LemonLoader.TinyJSON;
using LemonLoader.Tomlyn;
using LemonLoader.Tomlyn.Model;
using LemonLoader.Tomlyn.Syntax;
...
namespace LemonLoader.AssemblyGenerator
{
    internal static class Main
    {
        ...
        private static void PreSetup(string gameRoot)
        {
            ...
            AssemblyFolder = Path.Combine(gameRoot, "LemonLoader", "Managed");

            MSCORLIB_Path = Path.Combine(AssemblyFolder, "mscorlib.dll");
            
            BaseFolder = SetupDirectory(Path.Combine(Path.Combine(Path.Combine(gameRoot, "LemonLoader"), "Dependencies"), "AssemblyGenerator"));
