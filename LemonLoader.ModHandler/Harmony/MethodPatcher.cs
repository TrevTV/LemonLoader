...
namespace Harmony
{
	public static class MethodPatcher
	{
		...
		{
			return CreatePatchedMethod(original, ("LemonLoader_HARMONY_PATCH_" + LemonLoader.BuildInfo.Version), prefixes, postfixes, transpilers);
		}
		...
		public static DynamicMethod CreatePatchedMethod(MethodBase original, string harmonyInstanceID, List<MethodInfo> prefixes, List<MethodInfo> postfixes, List<MethodInfo> transpilers)
		{
			try
			{
				...
				bool isIl2Cpp = LemonLoader.UnhollowerSupport.IsGeneratedAssemblyType(original.DeclaringType);
